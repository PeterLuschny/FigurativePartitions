...
function renderPalette() {
  ...
  const oeis: Record<number,string> = {
...
function renderBoard() {
  ...
  for (const fig of figures) {
    ...
    if (fig.id === activeId) div.classList.add("active");
  ...
  if (won) {
    (document.getElementById("winSound") as HTMLAudioElement)?.play();
  }
}

function updateActionButtons() {
  const addBtn    = document.getElementById("btnAdd")    as HTMLButtonElement;
  const adjBtn    = document.getElementById("btnAdjust") as HTMLButtonElement;
  const rmBtn     = document.getElementById("btnRemove") as HTMLButtonElement;
...
document.getElementById("setTargetBtn")!
  .addEventListener("click", () => {
    const inp = document.getElementById("targetInput") as HTMLInputElement;
...
document.getElementById("board")!
  .addEventListener("click", ev => {
    const figEl = (ev.target as HTMLElement).closest(".figure") as HTMLElement | null;
    if (!figEl) return;
